...
- `real-katago-onnx-policy-v1`: proves the arena can call a real ONNX model and
  get legal moves with no fallback, but the current minimal encoder produces a
  weak capture-go player.
