...
| flutter-full-matrix-v1 | 2026-05-19 | all frameworks | all 8 current configs | Run the requested deterministic full matrix with real Flutter ONNX KataGo and no random opening | 8 configs, 28 unordered pairs, openings empty/cross/twist-cross, both first-player directions, repeat 2, board 9x9, capture target 5, max moves 120, non-KataGo timeout 5s, KataGo `timeBudgetMillis` 10000. Full-board no-legal positions are draws, not bot failures. | `flutter build web -t tool/flutter_full_matrix_arena_probe.dart`; `python3 -m http.server 8092 --bind 127.0.0.1 --directory build/web`; `FULL_MATRIX_SHARD_SIZE=3 node tool/capture_full_matrix_arena_probe.js`; reran `FULL_MATRIX_START_CELL=129 FULL_MATRIX_END_CELL=132 FULL_MATRIX_MERGE_EXISTING=1` after one transient ONNX runtime inference error; output `docs/ai_eval/runs/2026-05-19-flutter-full-matrix-arena-probe.json` | Final artifact: 168 cells / 336 games, random 0, illegal 0, timeout 0, fallback 0, failureReasons 0. Ranking by match wins: heuristic_counter_standard 30-8-4, mcts_counter_standard 29-10-3, heuristic_adaptive_weak 28-10-4, hybrid_tactical_counter_standard 26-12-4, hybrid_tactical_counter_weak 21-13-8, mcts_counter_weak 13-22-7, katago_onnx_standard 3-39-0, katago_onnx_weak 3-39-0. | Good full-matrix artifact | This is the first complete deterministic all-config comparison under the five-capture rule using Flutter ONNX KataGo without fallback. KataGo remains weak in this minimal adapter: both configs scored 6-78 by games and tied each other 6-6. |
| katago-capture-search-v1 | 2026-05-20 | katago | `katago_onnx_standard_v1` vs `katago_onnx_weak_v1` | Make KataGo standard meaningfully stronger than weak without fallback | Weak: real ONNX policy, `policyTemperature 1.35`, `candidateLimit 12`, `captureSearchDepth 0`. Standard: same real ONNX adapter, deterministic policy candidate, `captureSearchDepth 2`, 10s budget. | `dart run tool/headless_full_matrix_arena_probe.dart --configs katago_onnx_standard_v1,katago_onnx_weak_v1 --workers 2 --out .cache/katago-standard-vs-weak-tuned3.json` | Standard won 12-0 across 6 cells / 12 games, illegal 0, timeout 0, fallback 0, failureReasons 0. Each opening was 4-0. | Good active-goal slice | This intentionally prioritizes a strong standard-over-weak signal. It does not satisfy the older calibration TODO that wanted about 7-3 and weak comparable to MCTS standard. |
| mcts-capture-search-v1 | 2026-05-20 | mcts | `mcts_counter_standard_v1` vs `mcts_counter_weak_v1` | Strengthen MCTS standard beyond the previous 8-4 headless result without exceeding the non-KataGo 5s budget | Weak unchanged: adaptive beginner, 1 playout, depth 2, candidates 3, temperature 30, randomLegalMoveRate 0.25. Standard: counter intermediate, 12 playouts, depth 6, candidates 7, temperature 1.25, `captureSearchDepth 2`. | `flutter test test/ai_algorithm_framework_test.dart test/ai_arena_executor_test.dart`; `dart run tool/headless_full_matrix_arena_probe.dart --configs mcts_counter_standard_v1,mcts_counter_weak_v1 --workers 2 --out .cache/mcts-standard-vs-weak-tuned3.json` | Focused tests passed. Standard won 10-2 across 6 cells / 12 games, illegal 0, timeout 0, fallback 0, failureReasons 0. By opening: empty 3-1, cross 3-1, twistCross 4-0. | Good active-goal slice | This improves on the earlier headless 8-4 result while preserving validation gates. |
...
- `flutter-full-matrix-v1`: completes the requested deterministic
  full-matrix evaluation over all current configs. It produced a reproducible
  336-game artifact with no random openings, illegal moves, timeouts, fallback
  games, or failure reasons.
- `katago-capture-search-v1`: gives the real ONNX KataGo standard config a
  concrete capture-go tactical search parameter and verifies a 12-0 direct
  result over weak with no fallback, illegal moves, timeouts, or failures.
- `mcts-capture-search-v1`: strengthens MCTS standard from the earlier 8-4
  headless direct result to 10-2 while keeping the non-KataGo 5s timeout gate
  clean.
...
- Full-matrix first pass produced one transient Flutter ONNX
  `INFERENCE_ERROR Invalid rank for input: bin_input Got: 2 Expected: 4` in
  the KataGo-standard-vs-MCTS-standard twist-cross cell. Rerunning that shard
  completed cleanly and the final artifact has zero failure reasons. This
  should still be tracked as a runtime stability risk for long ONNX runs.
- MCTS advanced default attempt: setting `mcts_counter_standard_v1` to
  advanced tactical mode with 24 playouts, depth 8, candidates 8, and
  temperature 1.0 produced 2 decision timeouts in the headless direct matrix.
  It was rejected because non-KataGo configs must stay within the 5s budget.
