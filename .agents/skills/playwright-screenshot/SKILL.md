...
4. For Flutter Web verification in this repo, prefer a release static web build served by Python: `bash scripts/compile-web-worker.sh`, then `flutter build web --no-pub --release`, then `python3 -m http.server --directory build/web`.
...
1. Build the AI worker once for the current code: `bash scripts/compile-web-worker.sh`.
2. Build static web once for the current code: `flutter build web --no-pub --release`.
3. Start a new Python static server for `build/web`: `python3 -m http.server <port> --bind 127.0.0.1 --directory build/web`.
4. Add a cache-busting query value to the URL, such as `&shot=YYYY-MM-DD-HH-mm-SS`, when comparing iterative visual changes.
5. Run `scripts/browser_screenshot.sh`.
6. Stop the Python server with `Ctrl-C` in the PTY, or kill the exact process if it does not exit.
7. Use a new Python process for the next shot. Rebuild `build/web` whenever source code or assets changed.

The AI search worker is not generated by `flutter build web`; it is compiled
from `web/ai_search_worker.dart` by `scripts/compile-web-worker.sh` and then
copied from `web/` into `build/web/` by Flutter. If it is missing from the
served bundle, web AI turns fail with `Web Worker error`.
